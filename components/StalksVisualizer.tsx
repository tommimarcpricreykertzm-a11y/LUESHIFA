...
const StalksVisualizer: React.FC<StalksVisualizerProps> = ({ 
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative h-64 w-full max-w-2xl mx-auto flex items-end justify-center perspective-1000 select-none ${interactive ? 'cursor-col-resize touch-none' : ''}`}
    ...
    >
      <div 
        className={`flex items-end justify-center transition-all duration-300 ease-out ${
          visualSplitIndex !== null ? 'gap-24' : 'gap-1'
        }`}
      >
        {/* Left Bundle */}
      ...
      {interactive && hoverIndex !== null && splitIndex === null && (
         <div className="absolute bottom-[-40px] text-xs text-stone-400 font-serif pointer-events-none animate-bounce flex flex-col items-center">
            <span className="text-xl leading-none mb-1 text-accent">▲</span>
            <span className="bg-white/80 backdrop-blur px-2 py-1 rounded shadow-sm border border-stone-200">点击此处分策</span>
         </div>
      )}
    </div>
  );
